...
BOOST_AUTO_TEST_CASE(testCSAdapterEqFct) {
  ...
  // we also have to initialise the mapping (direct 1:1 from the input without
  // xml)
...
BOOST_AUTO_TEST_CASE(testWithMapping) {
  // This test is a bit redundant because the mapping is tested separately. Just
  // to see that it works after integration.

  // This is basically a stripped down copy of the test before, so we leave out
  // all comments and just comment what is different.
