...
struct CallbacksTestFixture {
    ...
    //~ void    reset_counters()
            ...
            // callbacks
    void    on_set_callback (mtca4u::ProcessArray<float> const & ) //~  < void (ProcessArray<float> const & ) >
            {
                ++_set_cb_counter;
                mydarray.fill_spectrum (0, 5.0);
            }
    void    on_get_callback (mtca4u::ProcessArray<float> & )       //~  < void (ProcessArray<float> & ) >
            {
                ++_get_cb_counter;
...
BOOST_FIXTURE_TEST_SUITE( test_operation, CallbacksTestFixture ) // operation check: return values matter
	...
	BOOST_AUTO_TEST_CASE( test_getset_nocb )
	{
        ...
        mydarray.fill_whole_spectrum(vec_test);
