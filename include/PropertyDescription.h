...
struct AutoPropertyDescription : public PropertyDescription,
  ...
  AutoPropertyDescription(ChimeraTK::RegisterPath const &source_ = "",
                          std::string location_ = "", std::string name_ = "",
                          bool hasHistory_ = true, bool isWriteable_ = true)
      : PropertyDescription(location_, name_),
        PropertyAttributes(hasHistory_, isWriteable_), source(source_) {}
  virtual bool operator==(PropertyDescription const &other) const override {
    if (other.type() == typeid(AutoPropertyDescription)) {
      ...
      return source == casted_other.source && location == other.location &&
             name == other.name &&
             static_cast<const PropertyAttributes *>(this)->operator==(
                 casted_other);
    } else {
      return false;
    }
...
struct ArrayDescription : public AutoPropertyDescription {
  ...
  ArrayDescription(ChimeraTK::RegisterPath const &source_ = "",
                   std::string location_ = "", std::string name_ = "",
                   DataType dataType_ = DataType::Auto, bool hasHistory_ = true,
                   bool isWriteable_ = true)
      : AutoPropertyDescription(source_, location_, name_, hasHistory_,
                                isWriteable_),
        dataType(dataType_) {}

  /// Convenience constructor from an auto property description, just add the
  /// type
  ArrayDescription(AutoPropertyDescription const &autoDescription,
                   DataType dataType_)
  ...
  virtual bool operator==(PropertyDescription const &other) const override {
    if (other.type() == typeid(ArrayDescription)) {
      ...
      return dataType == casted_other.dataType &&
             static_cast<const AutoPropertyDescription *>(this)->operator==(
                 casted_other);
    } else {
      return false;
    }
...
struct LocationInfo : public PropertyAttributes {
  ...
  LocationInfo(bool useHasHistoryDefault_ = false,
      ...
      : useHasHistoryDefault(useHasHistoryDefault_),
        useIsWriteableDefault(useIsWriteableDefault_),
