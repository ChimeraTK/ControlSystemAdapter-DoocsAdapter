...
namespace ChimeraTK {
  ...
  class DoocsUpdater: public boost::noncopyable {
    ...
    public:
      ...
      // update should be specified. The lock is held while the updaterFunction is called, so it must neither obtained
      // nor freed within the updaterFunction.
      void addVariable( const ChimeraTK::TransferElementAbstractor &variable, EqFct *eq_fct, std::function<void ()> updaterFunction);
