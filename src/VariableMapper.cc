...
namespace ChimeraTK{
  ...
  bool VariableMapper::getHasHistoryDefault(std::string const & locationName){
    ...
    if (locationInfo.useHasHistoryDefault){
      std::cout << "setting hasHistory default from location info for "<< locationName << " to " << locationInfo.hasHistory << std::endl;
      return locationInfo.hasHistory;
    }else{
      std::cout << "locationInfo does not have default settings. setting global hasHistory default of to " <<  _globalDefaults.hasHistory << std::endl;
