...
namespace ChimeraTK {
  ...
  typename boost::shared_ptr<D_fct> DoocsPVFactory::createDoocsScalar(
    ...
    // Histories seem to be supported by DOOCS only for property names shorter than 64 characters, so disable history
    // for longer names. The DOOCS property name is the variable name without the location name and the separating slash
    // between location and property name. One has to subtract another 6 characters because Doocs automatically adds
